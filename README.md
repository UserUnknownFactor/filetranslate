...
Tool for entire process of collaborative game translation from Japanese to English
...
```Text
usage: filetranslate [-h] 
		[-e encoding] [-p file_patterns]
		[-g game_engine] [-lang game_language] [-gd game_files_path] 
		[-cm cut_mark]
		[-ra attr_regexp] [-rs text_regexp] [-rt tag_regexp]
		[-i | -u | -ocr | -t | -tu | -fix | -cut [N] | -a [mode]] 
		[-rit] [-o old_regexp] [-n new_replacer] 
		[-ca | -isc [type] | -isa [type]] 
...
1. Using `-ra`, `-rs`, `-rt`, `-e`, `-p` and `-i` parameters of `filetranslate` extract strings to be tranlated from game files into `%filename%_attributes.csv` and `%filename%_strings.csv` files.  
Alternatively use an entry in `game_regexps.csv` for the game engine patterns and use `-g <game_engine>` parameter with the registered engine (may require existing regular expressions fine-tuning). You can also create `%game_engine%.project` file in the project folder to automatically pick the engine every time you run `filetranslate` from that folder. The project file's first line can contain game path for `-gd` option (not recommended for transferred projects).  
...
1. Use  and `-t` parameter to perform machine translation of strings and recognized images' strings.
...
1. Perform `-isa` run to apply intersecting strings back to translation databases. Use 2 as parameter to apply string translation file intersections.
...
1. Move `translation_out` content to the game folder with overwriting or run a game specific steps to enable the translation.
1. Test run the game and fix translation `.csv` files again.
1. Repeat previous five steps until the game works as expected.
...
The program is currently sensitive to line separator type so there should be only Unix-type separators (LF) in the translation files. AutoCRLF otion of git should be `none` because of that.
...
1. Revert translation databases back with `-rx` parameter before applying or fixing them.
...
4. Perform corresponding followup steps from the main list.
...
1. Copy `translation_out` content to the game folder or run a game specific steps to enable the translation.
...
* `game_regexps.csv`: DSV file with game-specific file encodings, file masks and RegExp patterns;
  ...
  * The game database is searched in the project folder first and in this module folder second.
  ...
  * The main text RegExp should contain consequent capturing groups for (preamble(context)preamble)?(main text)(...). The main text and context are detected by their group names. The capture groups should capture the entire data block with no uncaptured elements. Only context can be embedded group.
  * Other RegExps should containg one capture group for the main content.
...
* `translation_dictionary_out.csv`: DSV file with replacements in RegExp format for `-fix` and `-a` options, in the latter case it's applied to the original file;

* `intersections.csv`: DSV file with attributes automatically found in multiple `_attribute.csv` files.
